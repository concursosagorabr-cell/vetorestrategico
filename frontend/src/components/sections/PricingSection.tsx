...
export const PricingSection: React.FC = () => {
  return (
    <section className="py-20 bg-background relative overflow-hidden">
      ...
      <ParallaxWatermark text="PLANOS &bull; INVESTIMENTO &bull; ROI" speed={0.16} direction="right" variant="light" />
      <StrategicVectorMesh theme="light" />

      {/* Background glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-4xl h-96 bg-emerald-500/5 blur-3xl pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-stretch">
          {PRICING_PLANS.slice(0, 3).map((plan, idx) => {
            ...
            return (
              <ParallaxLayer key={plan.id} speed={speed} className="h-full">
                <div
                  className={`h-full rounded-3xl p-8 flex flex-col justify-between transition-all duration-300 relative ${
                    plan.popular
                      ? "bg-white border-2 border-emerald-500 shadow-xl shadow-emerald-500/15 lg:-translate-y-2 hover:shadow-2xl"
                      : "bg-white/95 backdrop-blur-xs border border-slate-200 hover:border-slate-300 shadow-sm hover:shadow-lg"
                  }`}
                >
                  {/* Popular Badge */}
                  {plan.popular && (
                  ...
                  <div className="pt-8 mt-6 border-t border-slate-100">
                    <Button
                      ...
                      size="lg"
                      className="w-full justify-center text-sm font-bold shadow-xs hover:scale-105 active:scale-95 transition-all"
